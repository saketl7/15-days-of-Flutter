...
Ultimately, all these packages help you **separate your business logic from the UI code**. Here's a complete tutorial showing how to refactor a simple app for better separation of concerns, immutability, and type safety using [Freezed](https://pub.dev/packages/freezed) & [StateNotifier](https://pub.dev/packages/state_notifier):
...
To overcome some of the inherent issues with passing arguments to named routes, packages such as [auto_route](https://pub.dev/packages/auto_route) have been created. These packages leverage code-generation to provide a more type-safe and user-friendly navigation API.
...
![User Input](https://pbs.twimg.com/media/Ev5UPMxUYAA4P9-?format=jpg&name=small)
...
There's also a section about [other resources](https://flutter.dev/docs/development/ui/animations#other-resources), which points to additional samples, recipes and documentation that you can browse.
...
You can use the [simple_animations](https://pub.dev/packages/simple_animations) package to more easily create custom animations with less boilerplate code. This includes additional features such as [Stateless animations](https://pub.dev/packages/simple_animations#-stateless-animation-documentation) and [MultiTween]().
